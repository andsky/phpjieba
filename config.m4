...
if test "$PHP_JIEBA" != "no"; then
  ...
  dnl ])
  dnl
  dnl PHP_JIEBA_CFLAGS="-I@ext_srcdir@/include/cjieba/lib/ -I@ext_srcdir@/include/cjieba/deps/"
  libjieba_src="include/cjieba/lib/jieba.cpp"
  LDFLAGS="$LDFLAGS -lstdc++ -lm"
  dnl PHP_SUBST(JIEBA_SHARED_LIBADD)
  PHP_REQUIRE_CXX()
  PHP_NEW_EXTENSION(jieba, jieba.c $libjieba_src, $ext_shared)
  PHP_ADD_INCLUDE("$ext_srcdir/include/cjieba/lib")
  PHP_ADD_INCLUDE("$ext_srcdir/include/cjieba/deps")
